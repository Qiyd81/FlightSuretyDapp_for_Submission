...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it(`(constructor) checks first airline creation in contract deploy`, async function () {
    ...
    assert.equal(airline[0], 'First Airline', 'Incorret name of first airline');
    assert.equal(airline[1], true, 'First airline not registered');
    assert.equal(airline[2], 0, "First airline should't have investements before fund");
    assert.equal(airline[3], 10000, "First airline voteCount wasn't correctly assigned 10000");
  });
  ...
  it(`(multiparty) can block access to functions using requireIsOperational when operating status is false`, async function () {
    ...
    // await truffleAssert.reverts(config.flightSuretyData.authorizeAppContract(config.flightSuretyApp.address));
  ...
  it('(config) can authorize App Contract to call Data Contract functions', async () => {
    ...
    // try {
    //     await config.flightSuretyData.authorizeAppContract(config.flightSuretyApp.address);
    //     isAuthorized = config.flightSuretyData.authorizedAppContracts[config.flightSuretyApp.address];
    //     // isAuthorized = true;
    // }
    // catch(e) {
    //     console.log(e);
    // }
    // await config.flightSuretyData.authorizeAppContract(config.flightSuretyApp.address);
    let isAuthorized = await config.flightSuretyData.isAppContractAuthorized.call(config.flightSuretyApp.address, {from: config.flightSuretyApp.address});
  ...
  it('(airline) add funds to first Airline', async () => {
    ...
    // await truffleAssert.passes(config.flightSuretyApp.fund(config.firstAirline, {from: config.firstAirline, value: airlineInitialFund}));
    // const isRegistered = await config.flightSuretyData.isAirlineRegistered.call(config.firstAirline, {from:config.firstAirline});
    // console.log(isRegistered);
    await config.flightSuretyApp.fund.call(config.firstAirline, {from: config.firstAirline, value: airlineInitialFund});
